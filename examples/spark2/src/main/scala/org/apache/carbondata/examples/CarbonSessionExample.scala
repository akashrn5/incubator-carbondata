...
//    spark.sql("drop table if exists com")
//    spark.sql("create table com(test string, mobile struct<imei:string,imsi:string>, pod array<string>) stored by 'carbondata' tblproperties('local_dict_enable'='true','local_dict_columns'='test','local_dict_threshold'='3000')")
//    spark.sql("load data inpath '/home/root1/Documents/data.csv' into table com options('COMPLEX_DELIMITER_LEVEL_1'='$', 'COMPLEX_DELIMITER_LEVEL_2'=':')")
//    spark.sql("select * from com").show()
    spark.sql("describe formatted com").show()
  }
}