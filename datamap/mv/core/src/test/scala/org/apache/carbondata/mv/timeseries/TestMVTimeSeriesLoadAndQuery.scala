...
//    checkPlan("datamap2", df3)
  ...
  test("test mv timeseries with join scenario") {
    ...
    sql(
      "create datamap datamap1 using 'mv_timeseries' as " +
      "select timeseries(maintable.projectjoindate,'month') ,sum(maintable.salary),sum(secondtable.projectcode) from maintable,secondtable  where maintable.salary LIKE '3%' and secondtable.projectcode = 5 group by timeseries(maintable.projectjoindate,'month')")
    val df = sql("select timeseries(maintable.projectjoindate,'month') ,sum(maintable.salary),sum(secondtable.projectcode) from maintable,secondtable  where maintable.salary LIKE '3%' and secondtable.projectcode = 5 group by timeseries(maintable.projectjoindate,'month')")
    checkPlan("datamap1", df)
  }
  ...
  def loadData(): Unit = {
    sql(
      s"""LOAD DATA local inpath '$resourcesPath/mv_sampledata.csv' INTO TABLE maintable  OPTIONS
