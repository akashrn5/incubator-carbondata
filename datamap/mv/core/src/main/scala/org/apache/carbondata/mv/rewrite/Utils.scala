...
object Utils extends PredicateHelper {
  ...
  private def canBeDerived(subsumer: ModularPlan, expE: Expression): Boolean = {
    ...
    // write logic of getting all expression and making semantic equals, becase expr.child can be of Cast expression, so cast expression child will be Scala UDF
