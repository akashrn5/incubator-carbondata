...
class TestCTASWithSecondaryIndex extends QueryTest with BeforeAndAfterAll{
  ...
  override def beforeAll: Unit = {
    ...
    sql(
      "create index sc_indx1 on table carbon_table(c_phone,c_mktsegment) as 'org.apache.carbondata.format'")
    sql("create index sc_indx2 on table carbon_table(c_phone) as 'org.apache.carbondata.format'")
    ...
    sql("create index ind_i1 on table udfValidation (deptname, empname) AS 'org.apache.carbondata.format'")
    sql("create index ind_i2 on table udfValidation (designation) AS 'org.apache.carbondata.format'")
    sql(
      "create index ind_i3 on table udfValidation (workgroupcategoryname) AS 'org.apache.carbondata.format'")
    sql("create index ind_i4 on table udfValidation (deptno) AS 'org.apache.carbondata.format'")
    sql("create index ind_i5 on table udfValidation (deptname) AS 'org.apache.carbondata.format'")
  }
