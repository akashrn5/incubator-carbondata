...
class TestBroadCastSIFilterPushJoinWithUDF extends QueryTest with BeforeAndAfterAll {
  ...
  override def beforeAll: Unit = {
    ...
    // create index tables
    sql("create index ind_i1 on table udfValidation (workgroupcategoryname,empname) AS 'org.apache.carbondata.format'")
    sql("create index ind_i2 on table udfValidation (designation) AS 'org.apache.carbondata.format'")
    sql("create index ind_i3 on table udfValidation (workgroupcategoryname) AS 'org.apache.carbondata.format'")
