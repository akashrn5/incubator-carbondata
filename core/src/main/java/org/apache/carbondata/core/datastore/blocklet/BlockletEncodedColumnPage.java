    ...
    if (null != this.fallbackFutureQueue) {
      try {
        ...
        while (!fallbackFutureQueue.isEmpty()) {
          ...
          // check if it done , if not then wait for 100 milli seconds as looping is cpu
          // intensive process
          // if task is done add the encoded column page to list
          if (fallbackTask.isDone()) {
            FallbackEncodedColumnPage fallbackEncodedColumnPage = fallbackTask.get();
