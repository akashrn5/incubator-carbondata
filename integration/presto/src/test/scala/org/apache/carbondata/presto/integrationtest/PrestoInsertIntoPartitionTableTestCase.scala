...
class PrestoInsertIntoPartitionTableTestCase
  ...
  test("test insert to partition table with multiple partition columns") {
    ...
    val actualResult1: List[Map[String, Any]] = prestoServer
      .executeQuery("select count(*) AS RESULT from testdb.multipartitiontable WHERE country = 'India'")
  ...
  def getSchemaBuilder(isMultiplePartionColuns: Boolean): (TableSchemaBuilder, PartitionInfo) = {
    ...
    partitionColumnSchemas.add(schemaBuilder.addColumn(new StructField("country", DataTypes.STRING), integer, false, false))
    if (isMultiplePartionColuns) {
      partitionColumnSchemas.add(schemaBuilder.addColumn(new StructField("state", DataTypes.STRING), integer, false, false))
      partitionColumnSchemas.add(schemaBuilder.addColumn(new StructField("city", DataTypes.STRING), integer, false, false))
    }
